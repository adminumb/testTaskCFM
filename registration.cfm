...
<cfif structKeyExists(form, 'fld_newUserSubmit')>
	...
	<cfif form.userLogin EQ ''>
    	<cfset arrayAppend(aErrorMessages, 'Пожалуйста сначала залогиньтесь')/>
    </cfif> 	
    ...
    <cfif form.resultList EQ '0'>
     ...
     <cfif ArrayisEmpty(aErrorMessages)  >
    	...
    	<cfset form.fld_userPassword = generateSecretKey("AES") />
    		...
    		'#form.resultList#','#form.userDesc#','#form.userComment#',(select max(user_id) from user),'#form.title#')
...
<body>
  ...
  <div id="pageBody">
	...
	<div class="formContainer" id="calendarSideBar">
		...
		<cfquery datasource='mysqBD' name='lastUser'>
			select user_id, name from user where user_id=(select max(user_id) from user)
		</cfquery>
								<cfoutput> 		
